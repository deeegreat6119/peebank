...
export const createDeposit = async (depositData, token) => {
  try {
    // Improved account ID validation
    const validateAccountId = (id, fieldName) => {
    ...
    validateAccountId(depositData.account, 'account');
    ...
    const response = await fetch(`${baseUrl}/deposit`, {
      ...
      credentials: "include",
      body: JSON.stringify({
        account: typeof depositData.account === 'object' ? depositData.account.accountNumber : depositData.account,
        amount: amount,
    ...
    return {
      status: "success",
      data: {
        depositId: data.data?.depositId || data.id,
        reference: data.data?.reference || data.data?.depositId,
